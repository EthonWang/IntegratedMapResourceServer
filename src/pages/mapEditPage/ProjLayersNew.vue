  ...
  <div style="width: 100%">
    <div class="treeGroupBtns">
      ...
      <div class="treeBtns">
        ...
        <el-popconfirm title="确定删除全部图层吗？" @confirm="allLayerDelete()">
          <el-button
            slot="reference"
            class="treeBtn"
            type="text"
    ...
    <div @click.stop class="treeBox">
      ...
      >
        ...
        >
          ...
          <div style="display: flex">
            ...
            <div class="nameLayer" type="flex" align-items="baseline">
              ...
              <div v-else class="projectTitle" v-show="data.id != nowGroupId">
                <h4 :title="data.showName" @click="nowGroupId = data.id">
                  {{ data.showName }}
                </h4>
          ...
          <!-- 图层的按钮 -->
          <div v-if="data.nodeType == 'layer'" class="layerBtnGroup">
            <el-button
              size="mini"
              type="success"
...
export default {
      ...
      switch (data.type) {
        case "addLayer1": // data:{type:'',layer:{}}
          if (data.isReplace == undefined) {
            if (data.drag == undefined) this.addLayerToTree(data.layer);
          }
          break;
  ...
  methods: {
    ...
    addLayerToTree(layerData) {
      ...
      if (groupId.length > 1) {
        ...
        let index = this.layersTree.findIndex(
          (currentValue) =>
            currentValue.nodeType == "group" && currentValue.id == groupId
        );
        if (index !== -1) {
        ...
        // 没有分组的图层，因为是按顺序添加，所以添加在第一个有图层的组之前
        let index = this.layersTree.findIndex(
          (currentValue) =>
            "children" in currentValue && currentValue["children"].length !== 0
        );
    ...
    handleLayerDelete(row) {
      ...
      if (
        this.layersNameObject[layerKey] === 0 &&
        !row.metadata["mapbox:type"].includes("mb")
      ) {
    ...
    allLayerDelete() {
      if (this.layers.length > 0) {
        ...
        for (let i in this.layers) {
          ...
          if (
            this.layersNameObject[layerKey] === 0 &&
            !item.metadata["mapbox:type"].includes("mb")
          ) {
            // 背景没有source
    ...
    allGroupClose() {
      ...
      this.collapseIcon = this.allGroupExpand
    ...
    addGroups(data) {
      if (JSON.stringify(data) !== "{}") {
        ...
        for (let item in groupsInfo) {
          let index = this.layersTree.findIndex(
            (currentValue) =>
              currentValue.nodeType == "group" &&
              currentValue.showName == groupsInfo[item].name
          );
          if (index == -1) {
            let node = {
              id: item,
    ...
    copyLayer() {
      ...
      let aimNode =
        JSON.parse(JSON.stringify(this.$refs.tree.getCurrentNode())) || {};
    ...
    getLayerIndex(layerId) {
      for (let i = 0, len = this.layers.length; i < len; i++) {
        if (this.layers[i]["id"] == layerId) return i;
      }
    },
