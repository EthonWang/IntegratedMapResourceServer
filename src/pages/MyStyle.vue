            ...
            <el-table-column label="操作" min-width="120">
              <template slot-scope="scope">
                <el-row type="flex" justify="start">
                  <el-col :span="15">
                    <el-input v-model="contentList[scope.$index]" @change="fontItemChange($event,scope.$index)" placeholder="输入内容查看效果">
          ...
          >
            ...
            <div class="el-upload__tip" slot="tip">
              <el-row type="flex">
                <h3 style="display: inline-block; flex: 1">要求:</h3>
                <h3 style="display: inline-block; flex: 8">
                  上传sprite文件(png和json一起选中压缩为zip，zip名字即为sprite名字)
                  <br />!!!不要嵌套文件夹!!!
                </h3>
              </el-row>
            </div>
    ...
    >
      ...
      <div class="page-part">
        <div class="leftUpload">
          ...
          <el-empty v-if="!showSymbol" description="选取单项展示信息"></el-empty>          
          <div v-if="showSymbol">
            <el-card class="symbolShowBox" body-style="padding:0" shadow="never">
              <el-image
                v-if="isSymbolList" fit="fill" 
              ...
              <el-form 
                ...
                :model="symbolSelect" style="margin-top:20px">
                <el-form-item
                    label="名称:">
        ...
        <div class="rightContent">
          ...
          <el-card
            ...
            :body-style="{padding:0,'background-color': isSymbolList ? '' : 'aliceblue',height:'calc('+100+'% - '+77+'px)'}"
          >
            <!-- 标题 -->
            <template slot="header">
              <div class="search-box-data">
                ...
                &nbsp;&nbsp;
                <i v-if="isSymbolList" class="fa fa-list iconBtn" title="切换至图片"
                  style="font-size:35px" @click="isSymbolList=false"></i>
                <i v-if="!isSymbolList" class="el-icon-menu iconBtn" title="切换至列表"
                  style="font-size:35px" @click="isSymbolList=true"></i>
              </div> 
              <div v-if="!isSymbolList" class="btnBoxType">
                <el-button v-if="!isSymbolEdit" type="primary" size="middle" 
                  plain circle icon="el-icon-edit" @click="isSymbolEdit=true">
                </el-button>
                <el-button v-if="isSymbolEdit" type="danger" size="middle" @click="symbolDeletePatch">删除</el-button>
                <el-button v-if="isSymbolEdit"  type="primary" size="middle" @click="symbolDelCancle">返回</el-button>            
              </div>              
            </template>
            <!-- 图片展示 -->
            <div v-if="!isSymbolList" class="symbol-box">
              <div
                ...
                :key="index" :title="item.originName" class="symbolItemCard" 
                @click="clickSymbolImg(item,index)"
                :style="{
                  border: (index == symbolSelectIndex ? 'solid ' +1+'px '+'#909399' : 'solid ' +1+'px '+'#f0f8ff')
                }"
              >
                <el-checkbox 
                  v-if="isSymbolEdit" class="spriteCheck" 
            ...
            <div v-if="isSymbolList" class="symbolListBox">
              <el-table :data="symbolTableData" stripe row-key="id" :max-height="symbolTableHeight"
    ...
    <el-card
      ...
      <div class="page-part">
        <div class="leftUpload">
          ...
          <el-empty v-if="!styleItemShow" description="选取样式，展示配图信息"></el-empty>          
      ...
      <div class="page-part">
        <div class="leftUpload">
          ...
          <el-row type="flex" justify="space-around">
            <el-select v-model="mbTileSelectId" placeholder="" @change="getStyleListById($event)">
              <el-option
            ...
            <el-button 
              type="success"
              circle size="middle" class="el-icon-plus"
          ...
          >
            ...
            <span slot="footer" class="dialog-footer">
              <el-button @click="mbStyleEditShow = false"
                >取 消</el-button
              >
          ...
          >
            ...
            ></iframe>
            <span slot="footer" class="dialog-footer">
              <el-button @click="osmStyleEditShow = false"
                >取 消</el-button
              >
        ...
        <div class="rightContent">
          ...
          >
            ...
            <el-row type="flex" justify="start" style="flex-wrap:wrap">
              <div v-for="(item,index) in mbTielsStyleList" :key="index" 
                      class="templateCard">
...
export default {
  ...
  data() {
    return {
      ...
      typeStyleBases: {'circle':[],'line':[],'fill':[],'fill-extrusion':[],'symbol':[]},
      typeTranslate: {'circle':'点','line':'线','fill':'面','fill-extrusion':'三维填充','symbol':'图标'},
  ...
  methods: {
    ...
    getFontList() {
      let data = null;
      requestApi.getFontList().then((res) => {
        data = res.data.data;
      }).then(()=>{
        // 初始化各个字体样式展示的内容
        this.fontTempList = [];
    ...
    symbolDeletePatch(){
      this.$confirm('将删除所选图标, 是否继续?', '提示', {
        ...
        for(let item of List){
          requestApi.deleteSymbol(item.id)
              .then(() => {
                this.$message.success("删除成功!");
              })
              .catch((error) => {
    ...
    getMbtilesInfo(){
      requestApi.getMbtilesList()
        .then((res) => {
          ...
          this.mbSourceList = mbOriginList.filter(data=>!data.osmMbtilesBoolean);     //获取非mbTiles源数据列表
          this.mbSourceList.unshift({id:'OSM',name:'OSM (OpenStreetMap)'});       //首位添加OSM数据
          this.mbTileSelectId = JSON.parse(JSON.stringify(this.mbSourceList[0].id));
    ...
    getStyleListById(id){
      console.log(id);
      if(id == 'OSM'){
        requestApi.getOsmStyleTemplateList()
          .then((res) => {
    ...
    osmStyleDelete(id){
      this.$confirm('确认删除该文件, 是否继续?', '提示', {
        ...
        requestApi.deleteOsmStyleTemplate(id)
          .then((res)=>{
            if(res.data.code == 0){
