<template>
  <div class="box" >
    <el-row v-if="menuButtonShow">
      <el-button v-if="!attribute.includes('translate-anchor') && !attribute.includes('pitch-alignment') && !attribute.includes('pitch-scale')" 
      ...
      <el-popover
        ...
        <el-button v-if="!attribute.includes('translate-anchor') && !attribute.includes('pitch-alignment') && !attribute.includes('pitch-scale') && !attribute.includes('cap') && !attribute.includes('dasharray') && !attribute.includes('antialias') && attribute.slice(-9) != 'translate'"  
      ...
      <el-popover
        ...
        <span>选择字段</span>
        <el-table :data="dataOptions.filter(data => !search || data.value.toLowerCase().includes(search.toLowerCase()))" 
                  :cell-style="{textAlign:'left'}" @row-click="menuButtonShow = false;attributeShow = true">                                 
          <el-table-column
            prop="value"
            align="right">
            <template slot="header" >
        ...
        <el-button v-if="!attribute.includes('translate-anchor') && !attribute.includes('pitch-alignment') && !attribute.includes('pitch-scale') && !attribute.includes('cap') && !attribute.includes('dasharray') && !attribute.includes('antialias') && attribute.slice(-9) != 'translate' && attribute.slice(-9) != 'translate'"
      ...
      <el-button v-if="!attribute.includes('translate-anchor') && !attribute.includes('pitch-alignment') && !attribute.includes('pitch-scale') && !attribute.includes('cap') && !attribute.includes('dasharray') && !attribute.includes('antialias') && attribute.slice(-9) != 'translate' && !attribute.includes('join')" 
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <el-row v-if="attribute.includes('opacity')||attribute.includes('translate')||attribute.includes('dasharray')||attribute.includes('line-cap')|| attribute.includes('join')||attribute.includes('fill-antialias')">
              <span style="margin-left: 10px">{{ scope.row.value }}</span>
            </el-row>            
        ...
        <el-row v-if="attribute.includes('dasharray')"> 
          <el-row class="zoomDasharray">
            <div class="flexRowSpaceAround"
              ...
              <h4 v-else>虚部:</h4>
              <el-input-number v-model="layer.paint['line-dasharray'][index]"
                              @change="handlePaintChange(layer['id'],'line-dasharray',layer.paint['line-dasharray'])"
                              :min="0"
                              size="small"
                              label="描述文字">
              </el-input-number>&nbsp;
              <el-button type="danger" icon="el-icon-delete" size="mini" circle @click="layer.paint['line-dasharray'].splice(index,1)"></el-button>
            </div>
          </el-row>  
...
export default {
  ...
  data () {
    return {
      ...
      layerOrigin: [],
      zoomValueOrigin: [],
      tabName: {'circle-color':'颜色','circle-radius':'半径','circle-opacity':'透明度','circle-stroke-color':'边线颜色','circle-stroke-width':'边线宽度','circle-stroke-opacity':'边线透明度','circle-blur':'模糊','circle-translate':'平移','circle-translate-anchor':'平移参考','circle-pitch-alignment':'倾斜对齐','circle-pitch-scale':'倾斜缩放',
                'line-color':'颜色','line-width':'线宽','line-opacity':'透明度','line-dasharray':'虚线','line-gap-width':'线间隙','line-blur':'模糊','line-translate':'平移','line-translate-anchor':'平移参考','line-offset':'偏移','line-cap':'线帽','line-join':'线连接',
                'fill-color':'颜色','fill-opacity':'透明度','fill-outline-color':'边线颜色','fill-translate':'平移','fill-translate-anchor':'平移参考','fill-antialias':'抗锯齿'},
      numAttribute: ['circle-radius','circle-opacity','circle-stroke-opacity','circle-stroke-width','circle-stroke-opacity','circle-blur','line-width','line-opacity','line-gap-width','line-blur','line-offset','fill-opacity'],
      arrayAttribute: ['circle-translate','line-dasharray'],
  ...
  methods: {
    initInfo(){
      ...
      this.zoomValue[0].value = this.layerSelect[this.layoutOrpaint][this.attribute];
      this.zoomValue[1].value = this.layerSelect[this.layoutOrpaint][this.attribute];
    ...
    zoomEditFirm(){
      ...
      // 设置多级显示
      this.zoomCondition = ["interpolate",[this.Rate],['zoom']];
      for(let i in this.zoomValue){
        this.zoomCondition.push(Number(this.zoomValue[i]['zoom']));
        //js默认为string类型
        if(this.isNum){
          this.zoomCondition.push(Number(this.zoomValue[i]['value']));
          console.log('num',this.isNum)
        }
        else if(this.attribute.slice(-9) == 'translate'){
          ...
          this.zoomCondition.push(["literal",[value1,value2]]);
          console.log('array')
        }
        // else if(this.attribute.includes('cap')){
        //   this.zoomCondition.push(["literal",this.zoomValue[i]['value']]);
        //   console.log('cap')
        // }        
        else{
          this.zoomCondition.push(this.zoomValue[i]['value']);
          console.log('text')
        }
      }
      const value = this.zoomCondition;
