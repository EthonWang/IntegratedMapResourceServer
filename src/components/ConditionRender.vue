  ...
  <div class="box" >
    <el-row v-if="menuButtonShowList[attribute]">
      ...
      <el-popover
        ...
        trigger="click">
        <span>筛选字段</span>
        <el-table :data="propertyList.filter(data => !search || data.column_data.toLowerCase().includes(search.toLowerCase()))" 
                  row-key="id" :cell-style="{textAlign:'left'}" @row-click="dataOpen" height="250">          
          <el-table-column
            prop="column_name"
            align="right">
        ...
        <el-button v-if="!attribute.includes('translate-anchor') && !attribute.includes('pitch-alignment') && !attribute.includes('pitch-scale') && !attribute.includes('cap') && !attribute.includes('dasharray') && !attribute.includes('antialias') && attribute.slice(-9) != 'translate' && !attribute.includes('gradient')"  
                   :disabled="attribute == 'fill-outline-color' && layer['paint']['fill-antialias'] == false" slot="reference" class="menuButton" plain >按数据范围设置样式</el-button>
      </el-popover>  
      <!-- 属性条件popover样式 -->
      <el-popover
        placement="right"
        ...
        trigger="click">
        <span>选择属性</span>
        <el-table :data="propertyList.filter(data => !search || data.column_name.toLowerCase().includes(search.toLowerCase()))" 
                  row-key="id" :cell-style="{textAlign:'left'}" class="attList" @row-click="propOpen"
                  height="250">                                 
          <el-table-column
            prop="column_name"
            align="right">
    ...
    <el-row v-if="zoomShow">
      <el-button v-if="isNum || attribute.slice(-9) == 'translate' || attribute.includes('color')" style="margin-bottom:10px;padding-left:10px;padding-right:50%;width:100%;" @click="zoomRateShow = true">
      ...
      <el-dialog
        ...
        <el-select v-model="zoomRate" placeholder="插值类型" style="width:73%">
          <el-option
            v-for="item in [{value: 'linear',label: 'linear'},{value: 'step',label: 'step'},{value2: 'exponential',label2: 'exponential'},{value2: 'cubic-bezier',label2: 'cubic-bezier'}]"
            :key="item.value"
            :label="item.label"
            :value="item.value">
          </el-option>
        ...
        <span slot="footer" class="dialog-footer">
          ...
          <el-button type="primary" @click="zoomRateShow = false;zoomRender()">确 定</el-button>
        </span>            
      </el-dialog> 
      <el-table
        ref="tableList"
    ...
    <el-row v-if="dataShow">
      <el-button v-if="isNum || attribute.slice(-9) == 'translate' || attribute.includes('color')" style="margin-bottom:10px;padding-left:10px;padding-right:50%;width:100%;" @click="dataRateShow = true">
      ...
      <el-dialog
        ...
        <span slot="footer" class="dialog-footer">
          <el-button type="primary" @click="dataRateShow = false;dataEditFirm()">确 定</el-button>
        </span>            
      </el-dialog> 
      <el-table
        ref="tableList"
      ...
      <el-dialog
        ...
        <br>
        <el-row v-if="attribute.includes('color')" style="display:flex">         
          <el-input v-model="dataValue[dataEditIndex].value"
                    placeholder="something" style="width:80%">
            <template slot="prepend" >color:&nbsp;</template>
          </el-input>
          <el-color-picker
        ...
        <el-row v-if="attribute.includes('translate')"> 
          <el-input v-model="dataValue[dataEditIndex].value[0]"
                    placeholder="something" style="width:80%">
            <template slot="prepend" body-style="padding:0">x轴方向平移:</template>
            <template slot="append">px</template>
          </el-input> 
          <br><br>                 
          <el-input v-model="dataValue[dataEditIndex].value[1]"
                    placeholder="something" style="width:80%">
            <template slot="prepend" body-style="padding:0">y轴方向平移:</template>
            <template slot="append">px</template>
          </el-input>           
        ...
        <el-row v-if="attribute.includes('dasharray')"> 
          <el-row class="dataDasharray">
            <div class="flexRowSpaceAround"
              ...
              <h4 v-if="index %2 ==0">实部:</h4>
              <h4 v-else>虚部:</h4>
              <el-input-number v-model="dataValue[dataEditIndex].value[index]"
                              :min="0"
                              size="small"
                              label="描述文字">
              </el-input-number>&nbsp;
              <el-button type="danger" icon="el-icon-delete" size="mini" circle @click="dataValue[dataEditIndex].value.splice(index,1)"></el-button>
            </div>
          </el-row>  
          <el-button type="primary" round size="mini" @click="layer.paint['line-dasharray'].push(0)">添加虚线设置
          </el-button>                           
        ...
        <el-row v-if="attribute.includes('line-cap') || attribute.includes('join')" type="flex" align= "middle" > 
          <h4>{{tabName[attribute]}}:&nbsp;</h4>
          <el-select v-model="dataValue[dataEditIndex].value" placeholder="请选择">
            <el-option
                v-for="item in [{label:'平直 butt',value:'butt'},{label:'圆形 round',value:'round'},{label:'方形 square',value:'square'}]"
                :key="item.value"
                :label="item.label"
        ...
        <el-row v-if="attribute.includes('fill-antialias')" style="display:flex"> 
          <el-switch v-model="dataValue[dataEditIndex].value">
          </el-switch>                           
        </el-row>         
    ...
    <el-row v-if="propertyShow">
      ...
      <el-dialog
        ...
        <el-row v-if="propEditIndex != 0" style="margin-bottom:10px">
          <el-popover
            ...
            <el-row type="flex" justify="space-between">
              ...
              <el-col :span="4">
                <el-button size="mini" type="primary" @click="propFilterConfirm">确定</el-button>
              </el-col>
            </el-row>
              <br>            
            <el-input
              ...
              <br>          
            <el-table :data="propValueList" @selection-change="propFilterChange" row-key="id" 
                      :cell-style="{textAlign:'left'}" height="250">
              <el-table-column
                type="selection"
                width="50">
              </el-table-column>
          ...
          <el-input :value="propValue[propEditIndex].prop" v-popover:propValuePopover
                    placeholder="something" style="width:80%" >
            <template slot="prepend" body-style="padding:0">{{propSelect}}:</template>
          </el-input>              
        </el-row>
        <el-row v-if="attribute.includes('color')" style="display:flex">         
          <el-input v-model="propValue[propEditIndex].value"
                    placeholder="something" style="width:80%">
            <template slot="prepend" >color:&nbsp;</template>
          </el-input>
          <el-color-picker
        ...
        <span slot="footer" class="dialog-footer">
          <el-button  :disabled="propEditIndex == 0" @click="propEditDelete">删除</el-button>
          <el-button v-if="propEditIndex != 0" type="primary" @click="propEditFirm" :disabled="propValue[propEditIndex].prop.length == 0">确 定</el-button>
          <el-button v-if="propEditIndex == 0" type="primary" @click="propEditFirm">确 定</el-button>
        </span>            
      </el-dialog> 
    </el-row>
...
export default {
  ...
  data () {
    return {
      ...
      rateContent: {linear:'线性：在刚好小于和刚好大于输入的停止点对之间进行线性插值。',step:'通过评估由输入和输出值对定义的分段常数函数，产生离散的阶梯式结果。'},
  ...
  methods: {
    ...
    zoomEditDelete(){
      ...
      if(this.zoomValue.length == 1){
        this.menuButtonShowList[this.attribute] = true;
        [this.zoomShow,this.dataShow,this.propertyShow,this.formulaShow] = [false];
        this.$bus.$emit("show",{param1:this.attribute,param2:true});
        const value2 = this.zoomValueOrigin;
        this.zoomValue = value2;  
        const value1 = this.zoomValueOrigin[0]['value'];   
    ...
    zoomRender(){
      ...
      if(this.zoomRate == 'step' || (!this.isNum && !this.isArray && !this.attribute.includes('color'))){
        for(let i in this.zoomValue){
    ...
    dataInsert(){
      ...
      const data = (Number(this.dataValue[index-1].data)+Number(this.dataValue[index].data))/2;
    ...
    dataEditFirm(){
      ...
      // 设置多级显示
      let dataCondition1 = ["interpolate",[this.dataRate],['get',this.dataSelect]];
      ...
      if(this.dataRate == 'step' || (!this.isNum && !this.isArray && !this.attribute.includes('color'))){
        for(let i in this.dataValue){
    ...
    //propperty变化处理
    handleTagClose(tag){
      this.propValueFilter[this.propEditIndex].splice(this.propValueFilter[this.propEditIndex].indexOf(tag), 1);
      console.log('tag',tag);
    ...
    propEditDelete(){
      ...
      if(this.propValue.length == 1){
        this.propEditIndex = 1;
        this.menuButtonShowList[this.attribute] = true;
        [this.propShow,this.dataShow,this.propertyShow,this.formulaShow] = [false];
        this.$bus.$emit("show",{param1:this.attribute,param2:true});
        const value2 = this.propValueOrigin;
        this.propValue = value2;  
        const value1 = this.propValueOrigin[0]['value'];   
    ...
    propFilterConfirm(){
      ...
      for(let i in this.propValueFilter[this.propEditIndex]){
        const value = this.propValueFilter[this.propEditIndex][i][this.propSelect];
        if(i == 0){
          string = value;
        }else{
          string = string + "," + value;  
